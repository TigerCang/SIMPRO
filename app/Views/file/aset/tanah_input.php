...
<div class="page-body">
    ...
    <div class="row">
        <div class="col-sm-12">
            ...
            <div class="card">
                <div class="card-header <?= ($tanah ? lang('app.bgDetil') : lang('app.bgInput')) ?>">
                    <h5><?= ($tanah ? lang('app.detildata') : lang('app.inputdata')) ?></h5>
                ...
                <div class="card-block mt-2">
                    ...
                    <input type="hidden" class="form-control" id="niaktif" name="niaktif" value="<?= (($tanah && $tanah[0]->is_aktif == '0') ? '1' : '0') ?>">
                    <input type="hidden" class="form-control" id="idperusahaan" name="idperusahaan" value="<?= ($tanah[0]->perusahaan_id ?? '') ?>">
                    <input type="hidden" class="form-control" id="idwilayah" name="idwilayah" value="<?= ($tanah[0]->wilayah_id ?? '') ?>">
                    <input type="hidden" class="form-control" id="iddivisi" name="iddivisi" value="<?= ($tanah[0]->divisi_id ?? '') ?>">
                    <input type="hidden" class="form-control" id="idwilayahlama" name="idwilayahlama" value="<?= ($tanah[0]->wilayah_id ?? '') ?>">
                    <input type="hidden" class="form-control " id="umur" name="umur" value="<?= ($tanah[0]->umur ?? '0') ?>">
                    <input type="hidden" class="form-control " id="sisa" name="sisa" value="<?= ($tanah[0]->sisa ?? '0') ?>">
                    <input type="hidden" class="form-control" name="gambarlama" value="<?= ($tanah[0]->gambar ?? 'default.png') ?>">
                    <div class="form-group row">
                        <label for="kode" class="col-sm-1 col-form-label"><?= lang('app.kode') ?></label>
                        <div class="col-sm-4">
                            <input type="text" harusisi <?= (($tanah && $tanah[0]->is_confirm != '0') ? 'readonly' : '') ?> class="form-control text-uppercase" id="kode" name="kode" maxlength="10" placeholder="<?= lang('app.min10kar') ?>" value="<?= ($tanah[0]->kode ?? '') ?>">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-11">
                            <input type="text" harusisi class="form-control" id="nama" name="nama" placeholder="<?= lang('app.harusisi') ?>" value="<?= ($tanah[0]->nama ?? '') ?>">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-11">
                            <select id="perusahaan" class="js-example-basic-single" name="perusahaan" <?= (($tanah && $tanah[0]->is_confirm != '0') ? 'disabled' : '') ?>>
                                <option value="" selected disabled><?= lang('app.pilih-') ?></option>
                                <?php foreach ($perusahaan as $db) : ?>
                                    <option value="<?= $db->id ?>" <?= (($tanah && $tanah[0]->perusahaan_id == $db->id) ? 'selected' : '') . ($tuser['act_perusahaan'] == '1' || preg_match("/(^|,)" . $db->id . "(,|$)/i", $tuser['perusahaan']) ? '' : 'disabled') ?>><?= "{$db->kode} => {$db->nama}" ?></option>
                                <?php endforeach; ?>
                            </select>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-4">
                            <select id="wilayah" class="js-example-basic-single" name="wilayah" <?= ($tanah ? (($tanah[0]->is_confirm != '0' && $tuser['act_super'] == '0') ? 'disabled' : '') : '') ?>>
                                <option value="" selected disabled><?= lang('app.pilih-') ?></option>
                                <?php foreach ($wilayah as $db) : ?>
                                    <option value="<?= $db->id ?>" <?= (($tanah && $tanah[0]->wilayah_id == $db->id) ? 'selected' : '') . ($tuser['act_wilayah'] == '1' || preg_match("/(^|,)" . $db->id . "(,|$)/i", $tuser['wilayah']) ? '' : 'disabled') ?>><?= $db->nama ?></option>
                                <?php endforeach; ?>
                            </select>
                        ...
                        <div class="col-sm-4">
                            <select id="divisi" class="js-example-basic-single" name="divisi" <?= (($tanah && $tanah[0]->is_confirm != '0') ? 'disabled' : '') ?>>
                                <option value="" selected disabled><?= lang('app.pilih-') ?></option>
                                <?php foreach ($divisi as $db) : ?>
                                    <option value="<?= $db->id ?>" <?= (($tanah && $tanah[0]->divisi_id == $db->id) ? 'selected' : '') . ($tuser['act_divisi'] == '1' || preg_match("/(^|,)" . $db->id . "(,|$)/i", $tuser['divisi']) ? '' : 'disabled') ?>><?= $db->nama ?></option>
                                <?php endforeach; ?>
                            </select>
            ...
            <div class="card">
                ...
                <div class="card-block mt-2">
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <select id="kategori" class="js-example-tokenizer" name="kategori">
                                ...
                                <?php foreach ($kattanah as $db) : ?>
                                    <option value="<?= $db->kategori ?>" <?= (($tanah && $tanah[0]->kategori == $db->kategori) ? 'selected' : '') ?>><?= $db->kategori ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="col-sm-10">
                            <select id="kelakun" class="js-example-basic-single" name="kelakun">
                                ...
                                <?php foreach ($kelakun as $db) : ?>
                                    <option value="<?= $db->id ?>" data-umur="<?= formatkoma($db->nilai, 0) ?>" <?= (($tanah && $tanah[0]->kakun_id == $db->id) ? 'selected' : '') ?>><?= $db->nama ?></option>
                                <?php endforeach; ?>
                            </select>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-4">
                            <input type="text" class="form-control" id="faktur" name="faktur" value="<?= ($tanah[0]->bukti ?? '') ?>">
                        </div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-4">
                            <input type="text" harusisi class="form-control form-control-right autonumber" data-digit-after-decimal="2" data-a-sep="." data-a-dec="," id="nibeli" name="nibeli" placeholder="<?= lang('app.harusisi') ?>" value="<?= ($tanah[0]->ni_beli ?? '') ?>" onchange="hitungsusut()">
                            <div id="error" class="invalid-feedback errnibeli"></div>
                        </div>
                        <div class="col-sm-1"></div>
                        <label for="niresidu" class="col-sm-1 col-form-label"><?= lang('app.residu') ?></label>
                        <div class="col-sm-4">
                            <input type="text" class="form-control form-control-right autonumber" data-digit-after-decimal="2" data-a-sep="." data-a-dec="," id="niresidu" name="niresidu" value="<?= ($tanah[0]->ni_residu ?? '') ?>">
                        </div>
                    </div>
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <select id="msusut" class="js-example-basic-single" name="msusut">
                                ...
                                <?php foreach ($selnama as $db) : ?>
                                    <option value="<?= $db->nama ?>" <?= (($tanah && $tanah[0]->modsusut == $db->nama) ? 'selected' : '') ?>><?= lang('app.' . $db->nama) ?></option>
                                <?php endforeach; ?>
                            </select>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-4">
                            <input type="text" class="form-control form-control-right autonumber" data-digit-after-decimal="2" data-a-sep="." data-a-dec="," id="nisusut" name="nisusut" value="<?= ($tanah[0]->ni_susut ?? '') ?>">
                        </div>
                        <div class="col-sm-1"></div>
                        <label for="umurdet" class="col-sm-2 col-form-label"><?= lang('app.umuraset') ?></label>
                        <div class="col-sm-3">
                            <input type="text" class="form-control form-control-right" readonly id="umurdet" name="umurdet" value="<?= ($tanah ? ($tanah[0]->sisa . " / " . $tanah[0]->umur) : '') ?>">
                        </div>
                    </div>
                </div>
            </div> <!-- Akhir card -->
            ...
            <div class="card">
                <div class="card-block mt-2">
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <input type="text" readonly class="form-control" id="linkpo" name="linkpo" value="<?= ($tanah[0]->nolink ?? '') ?>">
                        </div>
                    </div>
                </div>
            </div> <!-- Akhir card -->
        ...
        <div class="col-sm-3">
            ...
            <div class="card">
                ...
                <div class="card-block mt-2">
                    <div class="form-group row">
                        <div class="col-sm-12">
                            <img src="/assets/fileimg/tanah/<?= ($tanah[0]->gambar ?? 'default.png') ?>" class="img-thumbnail img-preview mx-auto my-auto d-block">
                        </div>
                    </div>
                    <div class="form-group row">
                        <div class="col-sm-12">
                            <input type="file" class="form-control" id="gambar" name="gambar" onchange="previewImage()" />
                            <div id="error" class="invalid-feedback d-block errgambar"></div>
                        </div>
                    </div>
                </div>
            </div> <!-- Akhir card -->
            ...
            <div class="card">
                ...
                <div class="card-block mt-1">
                    <div class="form-group row">
                        ...
                        <div class="col-sm-11">
                            <textarea class="form-control" rows="3" id="lokasi" name="lokasi"><?= ($tanah[0]->lokasi ?? '') ?></textarea>
                        </div>
                    </div>
                    <div class="form-group row">
                        <label for="surat" class="col-sm-1 col-form-label"><?= lang('app.dokumen') ?></label>
                        <div class="col-sm-11">
                            <textarea class="form-control" rows="3" id="surat" name="surat"><?= ($tanah[0]->surat ?? '') ?></textarea>
                        </div>
                    </div>
                    <div class="form-group row">
                        <label for="catatan" class="col-sm-1 col-form-label"><?= lang('app.catatan') ?></label>
                        <div class="col-sm-11">
                            <textarea harusisi class="form-control" rows="3" id="catatan" name="catatan" placeholder="<?= lang('app.harusisi') ?>"><?= ($tanah[0]->catatan ?? '') ?></textarea>
                    ...
                    <div class="d-flex">
                        <div class="col-3">
                            <div class="page-header-title">
                                <div class="d-inline">
                                    <span><?= lang("app.upby") . ' : ' . ($tanah[0]->upby ?? '') ?></span>
                                </div>
                            </div>
                        </div>
                        <div class="col-3">
                            <div class="page-header-title">
                                <div class="d-inline">
                                    <span><?= lang("app.confby") . ' : ' . ($tanah[0]->coby ?? '') ?></span>
                                </div>
                            </div>
                        </div>
                        <div class="col-3">
                            <div class="page-header-title">
                                <div class="d-inline">
                                    <span><?= lang("app.acby") . ' : ' . ($tanah[0]->akby ?? '') ?></span>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    $(document).ready(function() {
        ...
        $('.btnsave').click(function(e) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    $('#kode, #nama, #perusahaan, #wilayah, #divisi, #kbli, #kelakun, #nibeli, #msusut, #catatan, #gambar').removeClass('is-invalid');
                    $('.errkode, .errnama, .errperusahaan, .errwilayah, .errdivisi, .errkbli, .errkelakun, .errnibeli, .errmsusut, .errcatatan, .errgambar').html('');
                    if (response.error) {
