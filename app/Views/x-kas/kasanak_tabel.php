...
<table id="tabelload3" class="table table-striped table-hover nowrap">
    <thead>
        <tr class="bghead">
            ...
            <th scope="col" width="10" data-orderable="false" <?= $phid ?>>&#11206;</th>
            <th scope="col" <?= $rhid ?>>nomor</th>
    ...
    <tbody>
        ...
        foreach ($biaya as $row) :
            ...
            <tr>
                ...
                <td><?= $row->catatan ?></td>
                <td class="text-center" <?= $phid ?>><?= ($row->in_pph == '1' ? '&#9417;' : '') ?></td>
                <td <?= $rhid ?>></td>
                <td <?= $ixhid . $stat ?>>
                    <?php if ($xx == 'b') : ?>
                        <button type="button" class="btn <?= lang('app.btncPajak') ?> btnpajak"><?= lang('app.btnPajak') ?></button>
                    ...
                    <div class="dropdown-primary dropdown"><?= lang('app.btnDropdown') ?>
                        <div class="dropdown-menu eddm dropdown-menu-right">
                            ...
                            <?php elseif ($xx == 'b') : ?>
                                <a class="dropdown-item eddi tambahdata" data-id="<?= $row->id ?>" data-pilih='harga'><?= lang('app.ubahharga') ?></a>
                                <a class="dropdown-item eddi tambahdata" data-id="<?= $row->id ?>" data-pilih='jumlah'><?= lang('app.ubahjumlah') ?></a>
                            <?php endif; ?>
                        </div>
                    </div>
                </td>
...
<script>
    ...
    function pajak(id, deskripsi) {
        $.ajax({
            url: "/cekkas/pajakitem",
            type: 'post',
            data: {
                id: id,
                biaya: deskripsi,
    ...
    $('.tambahdata').click(function(e) {
        e.preventDefault();
        var getID = $(this).data('id');
        var getPilih = $(this).data('pilih');
        $.ajax({
            url: "/keuangan/modaltambah",
            data: {
                id: getID,
                pilih: getPilih,
            },
