...
class Keuangan extends BaseController
{
    ...
    {
        if ($this->request->isAJAX()) {
            $db1 = $this->deklarModel->satuID('kas_anak', $this->request->getVar('id'), '', 'id');
            $data = [
                'anak' => $db1,
                'biaya' => $this->deklarModel->satuID('m_biaya', $db1[0]->biaya_id ?? '', '', 'id'),
                'pilih' => $this->request->getVar('pilih'),
            ];
            $msg = ['data' => view('x-modal/inputtambahan_item', $data)];
            echo json_encode($msg);
        } else {
            exit('out');
        }
