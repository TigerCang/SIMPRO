...
class Loadtran extends BaseController
{
    ...
    public function tabelmintakas()
    {
        if ($this->request->isAJAX()) {
            ...
            $tgl_aw = tanggalmundur($this->request->getvar('tanggal'), '-1');
            $data = [
                'kas' => $this->tranModel->getKasinduk($this->urls[1], $this->user['id'], $this->request->getVar('status'), $levacc, '', $tgl_aw, $this->request->getvar('tanggal'), $perus, $wil, $div, $this->request->getVar('tujuan')),
                'menu' => $this->urls[1],
                'tujuan' => $this->request->getVar('tujuan'),
            ];
            $msg = ['data' => view('x-kas/mintakas_tabel', $data)];
            echo json_encode($msg);
        } else {
            exit('out');
        }
