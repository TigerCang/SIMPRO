...
class Biayasd extends BaseController
{
    ...
    {
        (!preg_match("/118/i", $this->menu['menu_1'])) && throw \CodeIgniter\Security\Exceptions\SecurityException::forDisallowedAction();
        $data = [
            't_menu' => lang("app.tt_sd"), 't_submenu' => '',
            't_icon' => '<i class="fa fa-file ' . lang("app.xlist") . '"></i>',
            't_diricon' => '<i class="fa fa-map-signs"></i>', 't_dir1' => lang("app.deklar"), 't_dir2' => lang("app.biayaproyek"), 't_dirac' => lang("app.sumberdaya"), 't_link' => '/biayasd',
            'kategori' => $this->deklarModel->distBiayalv1('sumberdaya'),
            'menu' => 'biayasd',
    ...
    public function crany()
    {
        ...
        return redirect()->to('/biayasd/input/' . $idu);
    }
    ...
    public function showdata($idunik)
    {
        ...
        $data = [
            ...
            'menu' => 'biayasd', 'ahid' => '', 'khid' => 'hidden', 'lhid' => 'hidden', 'shid' => '',
            'btnhid' => ($db1 ? 'hidden' : ''),
    ...
    public function savedata()
    {
        if ($this->request->isAJAX()) {
            ...
            $rule_lev2 = ($level == "2" ? 'required' : 'permit_empty');
            $satuan = ($level == "2" ? $this->request->getVar('satuan') : '');
            $akunbiaya = ($level == "2" ? $this->request->getVar('noakun') : '');
            ...
            $indukID = "0";
            if (strlen($this->request->getVar('kode')) == "8") {
                if ($level != "1") {
                    $cekInduk = $this->deklarModel->cekIndukbiaya('sumberdaya', strtoupper($biayaInduk), $levelInduk, '0');
            ...
            if (!$this->validate($validationRules)) {
                ...
                if ($this->request->getVar('postaction') == 'save') {
                    $this->biayaModel->save([
                        ...
                        'level' => $level,
                        'akun_id' => $akunbiaya,
                        'is_jumlah' => ($this->request->getVar('voljum') == 'on' ? '1' : '0'),
                        'is_confirm' => $stconf,
    ...
    public function tabeldata()
    {
        if ($this->request->isAJAX()) {
            ...
            $data = [
                'biaya' => $this->deklarModel->getBiaya($this->urls[1], 'sumberdaya', $kate),
                'menu' => 'biayasd', 'lts' => '011',
            ];
