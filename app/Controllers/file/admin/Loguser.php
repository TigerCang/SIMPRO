...
class Loguser extends BaseController
{
    public function index()
    {
        ...
        $data = [
            't_menu' => lang("app.tt_loguser"), 't_submenu' => '',
            't_icon' => '<i class="icofont icofont-list ' . lang("app.xlist") . '"></i>',
            't_diricon' => '<i class="fa fa-user-secret"></i>', 't_dir1' => lang("app.admin"), 't_dirac' => lang("app.loguser"), 't_link' => '/loguser',
    ...
    public function tabellog()
    {
        if ($this->request->isAJAX()) {
            $username = ($this->request->getVar('hid') == 'hidden') ? $this->session->username : $this->request->getVar('username');
            $detil = ''; //$this->request->getVar('detil')
            $data = [
                'isilog' => $this->deklarModel->getLog($username, $this->request->getVar('isi'), $detil),
