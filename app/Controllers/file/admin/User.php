...
class User extends BaseController
{
    ...
    {
        (!preg_match("/104/i", $this->menu['menu_1'])) && throw \CodeIgniter\Security\Exceptions\SecurityException::forDisallowedAction();
        $cek = $this->deklarModel->satuID('m_user', session()->username, '', 'kode');
        // if ($cek[0]->atasan_id != '0') throw \CodeIgniter\Security\Exceptions\SecurityException::forDisallowedAction();
        $data = [
            't_menu' => lang("app.tt_user"), 't_submenu' => strtoupper(lang("app.verortu")),
            't_icon' => '<i class="fa fa-user ' . lang("app.xlist") . '"></i>',
            't_diricon' => '<i class="fa fa-user-secret"></i>', 't_dir1' => lang("app.admin"), 't_dirac' => lang("app.user"), 't_link' => '/user',
            'user' => $this->deklarModel->getUser($this->urls[1]),
            'menu' => 'user',
    ...
    public function savedata()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$this->validate($validationRules)) {
                $msg = [
                    'error' => [
                        'atasan' => $this->validation->getError('idatasan'),
    ...
    public function deletedata()
    {
        if ($this->request->isAJAX()) {
            ...
            $this->logModel->saveLog('Delete', $this->request->getVar('idunik'), $db1[0]->kode);
            $this->session->setFlashdata(['judul' => "{$db1[0]->kode}" . lang("app.judulhapus")]);
            $msg = ['redirect' => '/user'];
            echo json_encode($msg);
        } else {
            exit('out');
