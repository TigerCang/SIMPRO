...
class Barang extends BaseController
{
    ...
    {
        (!preg_match("/135/i", $this->menu['menu_1'])) && throw \CodeIgniter\Security\Exceptions\SecurityException::forDisallowedAction();
        $data = [
            't_menu' => lang("app.tt_item"), 't_submenu' => '',
            't_icon' => '<i class="fa fa-cube ' . lang("app.xlist") . '"></i>',
            't_diricon' => '<i class="fa fa-cubes"></i>', 't_dir1' => lang("app.item"), 't_dirac' => lang("app.sparepart"), 't_link' => '/barang',
            'menu' => 'barang', 'ihid' => '', 'bhid' => 'hidden',  //item bahan 
            'katbarang' => $this->deklarModel->distItem('m_barang', 'kategori', 'barang'),
    ...
    public function crany()
    {
        ...
        return redirect()->to('/barang/input/' . $idu);
    }
    ...
    public function showdata($idunik)
    {
        ...
        $data = [
            't_menu' => lang("app.tt_item"), 't_submenu' => '',
            't_icon' => '<i class="fa fa-cube ' . $ticon . '"></i>',
            't_diricon' => '<i class="fa fa-cubes"></i>', 't_dir1' => lang("app.item"), 't_dirac' => lang("app.sparepart"), 't_link' => '/barang',
            ...
            'menu' => 'barang', 'ihid' => '', 'shid' => 'hidden', 'nhid' => 'hidden', 'ahid' => '', //item bahan sekon nonstoks akun
            'btnhid' => ($db1 ? 'hidden' : ''),
    ...
    public function savedata()
    {
        if ($this->request->isAJAX()) {
            ...
            $rule_kode = ($db1 ? ($db1[0]->kode != strtoupper($this->request->getVar('kode')) ? 'required|is_unique[m_barang.kode]|min_length[13]' : 'required|min_length[13]') : 'required|is_unique[m_barang.kode]|min_length[13]');
            $stconf = (($db1 && $db1[0]->is_confirm != '0') ? '2' : '0');

            $validationRules = [
                'kode' => [
                    'rules' => $rule_kode,
                    'errors' => ['required' => lang("app.errblank"), 'is_unique' => lang("app.errunik"), 'min_length' => lang("app.errlength", [13])]
                ],
                'nama' => [
                    'rules' => 'required',
            ...
            if (!$this->validate($validationRules)) {
                ...
                if ($this->request->getVar('postaction') == 'save') {
                    ...
                    $this->barangModel->save([
                        ...
                        'use_serial' => ($this->request->getVar('sn') == 'on' ? '1' : '0'),
