...
class Katrating extends BaseController
{
    ...
    {
        (!preg_match("/147/i", $this->menu['menu_1'])) && throw \CodeIgniter\Security\Exceptions\SecurityException::forDisallowedAction();
        $data = [
            't_menu' => lang("app.tt_katrating"), 't_submenu' => '',
            't_icon' => '<i class="fa fa-object-group ' . lang("app.xlist") . '"></i>',
            't_diricon' => '<i class="fa fa-map-signs"></i>', 't_dir1' => lang("app.sdm"), 't_dirac' => lang("app.katrating"), 't_link' => '/katrating',
            'menu' => 'katrating', 'khid' => 'hidden',
            'divisi' => $this->deklarModel->getDivisi($this->urls[1], 'katrating'),
    ...
    public function crany()
    {
        ...
        return redirect()->to('/katrating/input/' . $idu);
    }
    ...
    public function savedata()
    {
        if ($this->request->isAJAX()) {
            ...
            $nama = $this->deklarModel->cekDivisi('katrating', $this->request->getVar('nama'), $this->request->getVar('idunik'));
