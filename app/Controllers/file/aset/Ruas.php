...
class Ruas extends BaseController
{
    ...
    {
        (!preg_match("/124/i", $this->menu['menu_1'])) && throw \CodeIgniter\Security\Exceptions\SecurityException::forDisallowedAction();
        $data = [
            't_menu' => lang("app.tt_ruas"), 't_submenu' => '',
            't_icon' => '<i class="fa fa-road ' . lang("app.xlist") . '"></i>',
            't_diricon' => '<i class="fa fa-sitemap"></i>', 't_dir1' => lang("app.cabangaset"), 't_dir2' => lang("app.proyek"), 't_dirac' => lang("app.ruas"), 't_link' => '/ruas',
        ...
        return view('file/aset/ruas_view', $data);
    }
    ...
    public function crany()
    {
        ...
        return redirect()->to('/ruas/input/' . $idu);
    }
    ...
    public function showdata($idunik)
    {
        ...
        (empty($data['ruas']) && empty($data['idu'])) && throw \CodeIgniter\Exceptions\PageNotFoundException::forPageNotFound();
        if ($data['proyek1']) {
            if ($this->user['act_perusahaan'] == "0" && !preg_match("/," . $data['proyek1'][0]->perusahaan_id . ",/i", $this->user['perusahaan'])) throw \CodeIgniter\Security\Exceptions\SecurityException::forDisallowedAction();
            if ($this->user['act_wilayah'] == "0" && !preg_match("/," . $data['proyek1'][0]->wilayah_id . ",/i", $this->user['wilayah'])) throw \CodeIgniter\Security\Exceptions\SecurityException::forDisallowedAction();
            if ($this->user['act_divisi'] == "0" && !preg_match("/," . $data['proyek1'][0]->divisi_id . ",/i", $this->user['divisi'])) throw \CodeIgniter\Security\Exceptions\SecurityException::forDisallowedAction();
        }
        if ($db1) $this->logModel->saveLog('Read', $idunik, "{$data['proyek1'][0]->kode} => {$db1[0]->kode} ; {$db1[0]->nama}", '-');
        return view('file/aset/ruas_input', $data);
    }
    ...
    public function savedata()
    {
        if ($this->request->isAJAX()) {
            ...
            $db2 = $this->deklarModel->satuID('m_proyek', $db1[0]->proyek_id ?? '', '', 'id');
            $savj = ($db1 ? lang("app.judulubah") : lang("app.judulsimpan"));
            $cekRuas = $this->deklarModel->cekRuas($this->request->getVar('idunik'), 'ruas', $this->request->getVar('kode'), $this->request->getVar('idproyek'));
            $rule_kode = ($cekRuas ? 'required|is_unique[m_ruas.kode]' : 'required|min_length[4]');
            $stconf = (($db1 && $db1[0]->is_confirm != '0') ? '2' : '0');

            $validationRules = [
                'kode' => [
                    'rules' => $rule_kode,
                    'errors' => ['required' => lang("app.errblank"), 'min_length' => lang("app.errlength", [4]), 'is_unique' => lang("app.errunik")]
                ],
                'nama' => [
                    'rules' => 'required',
            ...
            if (!$this->validate($validationRules)) {
                ...
                if ($this->request->getVar('postaction') == 'save') {
                    ...
                    $db1 = $this->deklarModel->satuID('m_ruas', $this->request->getVar('idunik'));
                    $db2 = $this->deklarModel->satuID('m_proyek', $db1[0]->proyek_id, '', 'id');
                    $this->logModel->saveLog('Save', $this->request->getVar('idunik'), "{$db2[0]->kode} => {$db1[0]->kode} ; {$db1[0]->nama}");
                    $this->session->setFlashdata(['judul' => "{$db2[0]->kode} => {$db1[0]->kode} ; {$db1[0]->nama} {$savj}", 'proyek' => $db1[0]->proyek_id]);
                }
                //Konfirmasi
                if ($this->request->getVar('postaction') == 'confirm') {
                    $this->ruasModel->save(['id' => $db1[0]->id, 'is_confirm' => '1', 'confirmed_by' => $this->user['id']]);
                    $this->logModel->saveLog('Confirm', $this->request->getVar('idunik'), "{$db2[0]->kode} => {$db1[0]->kode} ; {$db1[0]->nama}");
                    $this->session->setFlashdata(['judul' => "{$db2[0]->kode} => {$db1[0]->kode} ; {$db1[0]->nama}" . lang("app.judulkonf"), 'proyek' => $db1[0]->proyek_id]);
                }
                //Aktifasi
                if ($this->request->getVar('postaction') == 'aktif') {
                    ...
                    $this->logModel->saveLog('Active', $this->request->getVar('idunik'), "{$db2[0]->kode} => {$db1[0]->kode} ; {$db1[0]->nama} {$onoff}");
                    $this->session->setFlashdata(['judul' => "{$db2[0]->kode} => {$db1[0]->kode} ; {$db1[0]->nama} {$savj}", 'proyek' => $db1[0]->proyek_id]);
                }
                $msg = ['redirect' => '/ruas'];
            }
    ...
    public function tabeldata()
    {
        if ($this->request->isAJAX()) {
            ...
            $data = ['ruas' => $this->deklarModel->getRuas($this->urls[1], 'ruas', '', $proyek)];
            $msg = ['data' => view('x-file/ruas_tabel', $data)];
            echo json_encode($msg);
        } else {
            exit('out');
        }
