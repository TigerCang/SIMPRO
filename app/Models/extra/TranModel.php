...
class TranModel
{
    ...
    // // //     $builder->update();
    // // // }
    public function getNomordoc($table, $awal, $tahun)
    {
        $builder = $this->db->table($table);
    ...
    // ____________________________________________________________________________________________________________________________
    public function getKasinduk($menu, $user = false, $status = false, $level = false, $sama = false, $awal = false, $akhir = false, $perusahaan = false, $wilayah = false, $divisi = false, $tujuan = false)
    // status setuju =>0:new, c:belum sama, 1:save tunda, 2:proses cek/setuju, 3:revisi, 4:tolak, 5:batal, 6:gudang, 7:ok, 8:Purchase, 9:Selesai
    {
        $builder = $this->db->table('kas_induk a');
        $builder->select('a.*, b.kode as perusahaan, c.nama as divisi, d.nama as wilayah, e.kode as kodeuser, f.kode as kodepeminta, g.kode as proyek, g.paket as paketproyek, 
            h.kode as camp, h.nama as namacamp, j.kode as alat, j.nomor as nomoralat, j.nama as namaalat, k.kode as tanah, k.nama as namatanah, x.id as xlog');
        // if ($menu == 'mintabarang') {
        $builder->where("(a.user_id LIKE '$user' OR peminta_id LIKE '$user')");
        if ($status != 'all') $builder->where('a.status', $status);
        // }
        ...
        $builder->join('m_user e', 'a.user_id=e.id', 'left');
        $builder->join('m_user f', 'a.peminta_id=f.id', 'left');
        $builder->join('m_proyek g', 'a.beban_id=g.id', 'left');
        $builder->join('m_camp h', 'a.beban_id=h.id', 'left');
        $builder->join('m_alat j', 'a.beban_id=j.id', 'left');
        $builder->join('m_tanah k', 'a.beban_id=k.id', 'left');
        $builder->join('log_data x', 'x.idunik = a.idunik' . $strx, 'left');
    ...
    // // // public function getPurchaseMinta($idunik, $username)
    // // // {
    // // //     $builder = $this->db->table('po_minta');
    // // //     if (empty($idunik)) {
    // // //         $builder->where(['peminta' => $username]);
    ...
    public function getLogaksi($idunik, $menu, $pilihan)
    {
        ...
        $builder->select('a.*, b.nama as pegawai, c.kode as username, d.nama as divisipegawai');
